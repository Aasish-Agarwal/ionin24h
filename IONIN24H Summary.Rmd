...
voting_by_location
write.table(voting_by_location, "voting_by_location.csv",row.names=F,sep=",")
...
voted_but_not_nvited <- merge(voters , mailing_list_members, by="email", all.x=TRUE) 

voted_but_not_nvited <- voted_but_not_nvited %>% tbl_df() %>%
    filter( is.na(available) ) %>%
  select(email) %>%
    print()

write.table(voted_but_not_nvited, "voted_but_not_nvited.csv",row.names=F,sep=",")
...
people_who_left %>% filter(voted == 0) %>% select(email) %>% unique()
people_who_left %>% filter(voted == 1) %>% select(email) %>% unique()
...
write.table(voting_by_area, "voting_by_area.csv",row.names=F,sep=",")
```
