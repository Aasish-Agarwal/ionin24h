...
prepare_master_data_frame <- function(session, mailing_lists, team_groups, innovators, voters, ion_contacts ) 
{
  ...
  write.table(report_df, "report_df.csv",row.names=F,sep=",")
...
summarize_voting_by_location <- function (report_df) 
{
  ...
  write.table(report_df_for_location %>% mutate(pct=pct/100),
    "voting_by_location.csv",row.names=F,sep=",")
  
  return(report_df_for_location)
}

print(summarize_voting_by_location(report_df = report_df))
```

# Participation By Area
This section presents the summary of people who voted from different area like CAT, XTP etc.
```{r, echo=F}
summarize_voting_by_area <- function (report_df) 
{
  ...
  write.table(report_df_for_area %>% mutate(pct=pct/100),
    "voting_by_area.csv",row.names=F,sep=",")
  
  return(report_df_for_area)
}

print(summarize_voting_by_area(report_df = report_df))
```
...
write.table(report_df_for_innovators, "report_df_for_innovators.csv",row.names=F,sep=",")
...
print(innovator_list)
write.table(innovator_list, "innovator_list.csv",row.names=F,sep=",")
...
get_potential_participants <- function (voters, report_df) 
{
  ...
  write.table(potential_participants,
      "potential_participants.csv",row.names=F,sep=",")

  return(potential_participants)
}

print(get_potential_participants (voters = voters, report_df = report_df) )
...
get_list_for_correction <- function (report_df) {
  ...
  write.table(people_who_left, "people_who_left.csv",row.names=F,sep=",")
  return(people_who_left)
}

print(get_list_for_correction(report_df = report_df))
```
